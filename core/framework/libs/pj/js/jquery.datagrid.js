			...
			$target.addClass("pj-grid").on("mouseenter.dg", ".pj-table tbody tr", function () {
				...
				if (iCnt > 0) {
					if ($data.confirmation !== undefined) {
						...
						return FALSE;
					} else if ($data.custom !== undefined && $data.custom === true) {
						return false;
					} else {
						if (e && e.preventDefault) {
							e.preventDefault();
						...
						for (j = 0; j < bCnt; j++) {
							...
							switch (inst.settings.buttons[j].type) {
								...
								case "delete":
									...
									break;
								case "custom":
									if (inst.settings.buttons[j].custom_class != undefined && inst.settings.content.data[i][inst.settings.buttons[j].custom_class] != undefined) {
										$a.addClass("btn-primary btn-outline btn-sm m-l-xs " + inst.settings.content.data[i][inst.settings.buttons[j].custom_class]);
									} else {
			...
			if (inst.settings.paginator) {
				...
				if (inst.settings.paginator.actions && inst.settings.paginator.actions.length) {
					...
					for (i = 0, kCnt = 0, iCnt = inst.settings.paginator.actions.length; i < iCnt; i++) {
						var $custom_class = '';
						if (inst.settings.paginator.actions[i].custom_class !== undefined) {
							$custom_class = inst.settings.paginator.actions[i].custom_class;
						}
						$a = $("<a>", {
							"href": inst.settings.paginator.actions[i].url
						})
							.addClass("pj-paginator-action" + " " + $custom_class)
