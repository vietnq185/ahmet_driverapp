...
class pjZipStream {
    ...
    function __destruct() {
    	if (is_resource($this->zipFile) || $this->zipFile instanceof GdImage) {
            fclose($this->zipFile);
        }
        $this->zipData = NULL;
    }
    ...
    public function addFile($data, $filePath, $timestamp = 0, $fileComment = NULL, $compress = TRUE) {
        ...
        if ((is_resource($data) || $data instanceof GdImage) && get_resource_type($data) == "stream") {
        ...
        if ((!is_resource($this->zipFile) && !($this->zipFile instanceof GdImage)) && ($this->offset + $gzLength) > $this->zipMemoryThreshold) {
            $this->zipflush();
        }
    ...
    public function addLargeFile($dataFile, $filePath, $timestamp = 0, $fileComment = NULL)   {
        ...
        } else if ((is_resource($dataFile) || $dataFile instanceof GdImage) && get_resource_type($dataFile) == "stream") {
            $fh = $dataFile;
