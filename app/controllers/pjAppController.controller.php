...
class pjAppController extends pjBaseAppController
{
    ...
    public static function generateAlphaHash($length = 32) {
    ...
    public static function getWhatsappTemplateParameters($templateName, $data) {
        $configs = [
            'errinnerung_fahrer' => ['driver_name', 'date'],
            'driver_reminder'     => ['driver_name', 'date'],
            'reminder_en'     => ['drivername', 'date'],
            'errinnerung_fahrer'     => ['drivername', 'date'],
        ];
        if (!isset($configs[$templateName])) return [];
        
        $params = [];
        // Duyệt qua danh sách token đã định nghĩa TRƯỚC theo thứ tự trên Meta
        foreach ($configs[$templateName] as $tokenName) {
            $field = $tokenName == 'drivername' ? 'driver_name' : $tokenName;
            $value = $data[$field] ?? ''; // Lấy giá trị từ dữ liệu DB
            
            $params[] = [
                "type" => "text",
                "parameter_name" => $tokenName,
                "text" => (string)$value // Meta yêu cầu kiểu string
            ];
        }
        return $params;
    }
     ...
     */
    public static function session_start_timeout($timeout=86400, $probability=100, $cookie_domain='/') {
