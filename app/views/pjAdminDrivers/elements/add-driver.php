<div class="panel no-borders">
	<div class="panel-heading bg-completed">
		<p class="lead m-n"><?php __('infoAddDriverTitle')?></p>
	</div><!-- /.panel-heading -->
		...
		<form action="" method="post" id="frmCreate">
			...
			<div class="form-group">
				...
				<select name="locale_id" class="select-item form-control required" data-msg-required="<?php __('plugin_base_this_field_is_required', false, true);?>">
					...
					{
						?><option value="<?php echo $loc['id']; ?>" <?php echo $loc['id'] == $controller->getLocaleId() ? 'selected="selected"' : '';?>><?php echo pjSanitize::html($loc['name']); ?></option><?php
					}
					?>
				</select>
			</div>	
				...
				<div class="form-group">			
    				<?php foreach (__('_types_of_drive', true) as $k => $v) { ?>
    					<div class="radio radio-inline radio-primary">
    						<input type="radio" id="type_of_driver_<?php echo $k;?>" name="type_of_driver" value="<?php echo $k;?>"<?php echo $k == 'own' ? ' checked' : NULL; ?>>
			...
			<div class="form-group">
				...
				<textarea rows="3" class="form-control" name="general_info_for_driver"></textarea>
			</div>
